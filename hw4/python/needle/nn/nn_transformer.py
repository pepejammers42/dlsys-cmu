...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        # sfmx = self.softmax(self.matmul(q, k) / (q.dim ** 0.5))
        # the matmul impl they expect a@b^T 
        mtml = self.matmul(q, k) / (q_dim ** 0.5)
        # causal mask need to add first then softmax
        if self.causal:
            mtml += Tensor(self.create_causal_mask(queries_len, queries_len, device=self.device), device=self.device, dtype=self.dtype).broadcast_to((batch_size, num_head, queries_len, queries_len))
        probs = self.dropout(self.softmax(mtml))
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        # (1)
        qn = self.prenorm_q(q.reshape((batch_size * queries_len, q_dim)))
        kn = self.prenorm_k(k.reshape((batch_size * keys_values_len, k_dim)))
        vn = self.prenorm_v(v.reshape((batch_size * keys_values_len, v_dim)))
        # (2)
        q_prime = self.q_projection(qn).reshape((batch_size, queries_len, self.num_head, self.dim_head)).transpose((1,2))
        k_prime = self.k_projection(kn).reshape((batch_size, keys_values_len, self.num_head, self.dim_head)).transpose((1,2))
        v_prime = self.v_projection(vn).reshape((batch_size, keys_values_len, self.num_head, self.dim_head)).transpose((1,2))
        # (3)
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.pre_norm = LayerNorm1d(q_features, device=device, dtype=dtype)
    ...
    def forward(
        ...
        x += self.res_dropout(self.attn(x))
        tmp = self.pre_norm(x.reshape((batch_size * seq_len, x_dim)))
        tmp = self.ffn(tmp).reshape((batch_size, seq_len, x_dim))

        return x + self.res_dropout(tmp)
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.pos_embeds = Embedding(sequence_len, embedding_size, device=device, dtype=dtype)
        self.layers = [TransformerLayer(embedding_size, num_head, dim_head, hidden_size, dropout=dropout, causal=causal, device=device, dtype=dtype) for _ in range(num_layers)]
    ...
    def forward(
        ...
        pos = Tensor(list(range(seq_len)), device=x.device, dtype=x.dtype, requires_grad=False).reshape((seq_len, 1)).broadcast_to((seq_len, batch_size))
        pos_embed = self.pos_embeds(pos).transpose((0, 1))
        x = x + pos_embed
