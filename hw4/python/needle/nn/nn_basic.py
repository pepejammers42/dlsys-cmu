...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.training:
            # binary mask
            dist = init.randb(*x.shape, p=1-self.p, dtype='float32', device=x.device)
...
class BatchNorm2d(BatchNorm1d):
    ...
    def forward(self, x: Tensor):
        s = x.shape
        xp = x.transpose((1,2)).transpose((2,3)).reshape((s[0] * s[2] * s[3], s[1]))
        y = super().forward(xp).reshape((s[0], s[2], s[3], s[1]))
